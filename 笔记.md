...
**思考题：** 一个新项目，需要做的第一件事是什么？
![业务流程](./imgs/业务流程.png)
正确的项目启动思路永远都是从**业务**入手
总结业务需求、建立业务模型，通过业务模型来确定网站的业务流程，最后业务流程出发，才能确定页面关系与数量，业务分析的最后一步才是 ui 的布局

**什么是业务？** 就是所做项目需要做到什么，实现生命功能
...
将需求中所隐含的**业务路线**实际上与**页面**逻辑形成了某种特定的一一对应的关系，我们就应该使用这种对应关系，来实现网站的所有页面的开发
...
系统设计中的需求设计：《需求设计规范》注意，这里的需求不是业务需求。而是我们的**软件需求**，这份文档主要是用来定义软件的功能性和非功能性的需求
功能性需求(设计用例)：
...
> 项目创建： npx create-react-app react-travel --template typescript
...
**并且给我的启发在于，自己也可以像人家 antd 一样，自己来封装这类组件来使用 达到全员组件化的目的**

**老师推荐的比较优秀的 github 内容**
![老师推荐1](./imgs/老师推荐.png)
...
header, footer 都是一个文件夹，人家对于文件夹的命名似乎都是首字母小写。但是对于组件文件 都是以大写首字母命名
...
**组件化让拆分和组合非常方便**
这个项目导出组件的方法(组件都放在 component 里面，并且导出都是 export \* from ... 正是体现了组件化的精髓)
...
其实这里的命名还是挺有意思的，把人家 antd 定义好的名字直接更改了

注意 组件之间传递数据。 传递的数据要在子组件进行类型定义
。

> 在 ts 中 组件类型是 JSX.Element
> 在 ts 中 后端数据一般不确定，所以使用 any 类型

slice(.., ..)方法

```
slice() 方法返回一个新的数组对象，这一对象是一个有begin和end决定的原数组的浅拷贝(包括begin,不包括end)。原数组不被改变
...
这节内容简单，但是有个亮点需要我们注意：多余展示多个图片，我们可以自己创个数据，然后使用 map 方法来渲染，优势更大！代码更简洁！！
例如：
...
**【什么是路由】**
先说一下路由器： 路由器的作用是通过读取路由表，根据 tcp/ip 中的地址，然后把数据包按最佳路线传送到指定位置。而**数据传输路线的计算过程就是路由，routing**
![路由器](./imgs/路由器.png)
...
**【虚拟路由】**
React 网站使用的路由系统 其实**都是虚拟**的，与后端服务器的文件架构没有半毛钱关系。
...
url 中第一个片段 localhost 是域名；第二个片段 products，代表某项资源，这里只是产品；第三个片段 131415999 则代表参数。根据 restful 约定，资源片段后面紧跟着的参数是这个资源的`唯一标志`,也就是资源的 id。 这种 url 可以带有天然的语义。比如，我们的这个例子这个 url 所代表的含义就是“打开产品 id 为 131415999 的详情页面”。目前越来越多的框架都在接受这样的`url的片段组合`。而我们的 react-router 也使用了这种方式来处理参数(所以参数才会有 /:param 的操作？)

获取路由参数的方法： props.match.params

RouteComponentProps 用法：
...
并且既然要使用hoc，组件的命名也是有讲究的。
例如：原来组件的名字为DetailPage。现在改为DetailPageComponent
在最后导出的时候再改为DetailPage
...
这样也就不再引入 RouteComponentProps 了
...
这样使用 Link 后，不用自己再导入 history 钩子了
...
注意： 在 redux 中不管是 reducer 还是 action 都是纯函数，reducer 文件 最后输出的是一个函数，所以可以用匿名函数来充当
...
只是需知道， createStore 创建 store 的时候，帮我们自定义了 store 的方法（例如 store.getState()）通过 getState 获得的数据类型就是 languageState
。所以可以通过链式操作，获取 language 的数据。
例如
...
```
store.subscribe(() => {
  const storeState = store.getState();  // 回调函数里也有store.getState() 用来不断的更新当前组件的数据
  this.setState({
    language: storeState.language,
  })
})
...
3. store.subscribe() 要放在 constructor 或者 componentDidMount 里面，因为每个组件只需要订阅一次 store 就可以一直持续不断的接受 store 数据的更新推送了，放在 constructor 或 componentDidmount 可以降低系统开销
   (这节还是结合 git 上传的代码来看比较合适)

对了，这里的 antd 组件 Menu 只有最外层的 Menu 有人家给的 onClick 事件监听，可以监听到 Menu 包裹着的 Menu.Item key 的内容
...
#### 7.8【redux 重构】action 的拆分与统一 这一节内容很重要
...
通过 ts 通过强类型的定义完成 reducer 对 action 的纠错(这里是非常强大的体现了 ts 的纠错功能)

发现没！其实这里的 action creator 返回的是一个对象(action)

更细节的内容看 git 提交的代码

#### 7-9【redux 封装】在类组件中使用 react-redux(重点)

类型的定义 使用 type 关键字 来定义类型
...
mapStateToProps 对应一个函数，参数是 state 且这个函数返回对象
mapDispatchToProps 对应一个函数，参数是 dispatch 返回的也是一个对象，返回的这个对象中每一个字段都是一个 dispatch 的处理函数

对于使用这两个方法 定义组件的 props 类型 要用下面的方法注意：
...
调用数据或被包含的 dispatch 方法 用 this.props.[...]
...
在用 useSelector 之前，要对这个钩子做类型定义(ts
)
useSelector 就代替了 mapStataToProps
写一个自定义的 useSelector(解决数据与 store 深度绑定问题，这个问题会导致组件无法复用，而失去了使用钩子的意义)
![useSelector](./imgs/useSelector.png)

useDispatch 代替了 mapDispatchToProps

所以可见，无论是 react-router-dom 还是 react-redux。 钩子函数的使用都是非常简单且高效的。所以，全力面向钩子函数才是关键！
...
这里有个点要注意 📢：
在组件 componentDidMount 组件中进行异步请求，会报错 (
报错原因和组件的生命周期以及数据的加载时机有关。 我们来分析一下，组件在初始化的时候第一步调用的就是构造函数，在该组件中，我把 state 设置为了空数组；紧接着第二步，ui 组件被挂载，虚拟 dom 开始渲染，因此，我们的 render 会执行，但问题是我们的 api 访问操作在 componentDidMount 中进行，而这个函数需要等待组件 ui 挂载完毕才会被调用(所以在原来的 return jsx 前 换为 return 一个转菊花 就作为第一次 ui 挂载)，也就是说，我们的 jsx 会渲染三个空列表，而问题是我们相对应的子组件并没有处理空列表的能力。因此，我们要在 jsx 代码中对悬空数据做一定的预处理，这样才能避免未来可能会发生的错误(基础方法 转菊花，更多方法有待未来发现)
)
**悬空数据预处理 转菊花**
// 这里可以使用 antd 的转菊花功能
...
- API 请求放在组件中是否合理?
- 是否有必要把 api 请求转移到 redux store 中执行呢？
- 假如把 API 请求转移到 store 中执行会给我们带来什么好处呢？

Dry 原则 让 store 统一管理 api 请求， Don't repeat yourself

action 是一个对象，不是一个过程，无法放入 api 请求
reducer 是一个纯函数 不应该处理副作用 (api 请求就是一个不折不扣的副作用)

redux 提供了一个中间件： redux-thunk

#### 8-8【中间件】什么是中间件

封装 dispatch
middleware 的理论基础就是复合函数

复合函数式 redux 中间件的基础，而柯里化就是实现中间件落地的技术
...
redux-thunk 就是让 dispatch 多支持了一种类型即函数类型

**redux-thunk 中间件的安装方法**

> npm i redux-thunk (自带 ts 版本)

**Redux 中间件公式**
const middleware = (store) => (next) => (action) => { }

这一节的内容在 imooc React 17 电子教程上已经写得非常清晰了

使用中间件的目的是把异步请求的代码放在 store 里去(实质是放在 action creator 里)
...
1. 如何避免 redux 中的模板代码？哪里出现了模板代码？(例如 action 要配套常量字符串类型，以及 action creator 和对应 reducer 来解决 dispatch 出来的指令(简而言之，那个放置常量以及 action creator 的文件其实是冗余的))
2. ant design 中的 table 组件如何使用？
3. 什么是 redux-toolkit？
4. immer 是什么？
5. createSlice 的作用是什么？
6. 如何使用 createAsyncThunk 来处理异步逻辑？
7. 什么情况下使用 extraReducers？
8. 如何使用 pendind、fulfilled、以及 rejected action？

#### 9-2 章节总览

目前遇见的困境 各种各样的模板代码 (action、reducer 的模板代码太多了...)
...
- 使用 Flux，多数据仓库、多 store，失败
- redux 配合 Rxjs， 还是失败
- 使用 mobx 整体替换 redux，还是失败(项目成功，但 mobx 社区太小很多细节得不到处理)

2020 年初 redux 官网推出了一个插件 redux-toolkit(RTK)
**非常推荐使用**

- RTK 俨然成为了一个非常主流的 redux 架构模式
- 得到了 react 官方和 redux 官方的知识
- 如何我们在使用 create-react-app 脚手架启动项目的时候，选择了 redux，那么官方默认的 redux 架构就是 rtk

**第九章的内容**

1. 将会使用 MVC 的架构完成产品搜索页面以及产品详情页面
2. 会简单了解一下什么是 redux-toolkit,然后进行源码解读，剖析这个工具的所包含的内容
3. 使用 redux-toolkit 来把产品搜索页面以及产品详情页面从 mvc 架构转化为 redux 的架构
4. 对比新旧架构，讨论优缺点

#### 9-3【详情页面搭建 1】页面框架、详情与日期选择 -- 用最简单的架构 mvc 来实现

<!-- 今晚 这些可以好好看看 复习复习 -->

一个项目我先应该弄成 mvc 再变为 redux？ 还是直接就 redux 进行操作？

突然想到一个问题： 行内优先级 和 !important 优先级谁更高？
还有个 axios 的默认设置 header 这个明天来做点笔记
以及转菊花的原因 componentDidMount...

处理 html 字符串的显示，react 为了防止注入攻击，有一个特殊的 html 处理机制，用 react 的 api 来处理，api：dangerouslySetInnerHTML 使用对象的方式来显示 html，对象的名称\_\_html
...
#### 9-6 redux-toolkit 实战

**createReducer** 有了这个后，连模板代码也没有了，switch 也没有了

使用了 RTK 以后，就不需要分别创建对应的 reducer 和 action

安装 redux-toolkit: npm install @reduxjs/toolkit (全面支持 js，所以不必自己再手动安装)

slice 用途非常广泛

#### 9-9【综合运用】搜索页面

似乎搜索的本质就是 路由的切换，加 api 参数的不同转换罢了！(页数切换、关键词搜索)

#### 10-1 带着问题来学习

1. 什么是 DRY 原则？
2. 如何使用 ant design 中的表单组件进行数据验证
3. 什么是 http 状态码？常见的状态码有哪些？
4. 什么是 JWT？它的实现原理是什么？ **(重点)**
5. JWT 与 session 有什么区别？ **(重点)**
6. 如何 decode jwt？ **(重点)**
7. 如何持久化 redux 状态 **(重点)**
...
- JSON Web Token
- JWT 的作用是**用户授权(Authorization), 而不是用户的身份认证**
...
**JWT 彻底改变了用户授权与认证的过程**
(无状态登录)
通过使用 jwt 来替换浏览器的 cookie，因为 jwt 只负责处理授权，所以 jwt 信息只需要保存在客户端，而不需要被保存在服务器上，从而可以实现无状态登录，因为本质上来说 jwt 跟登陆根本就没啥关系

![有状态登录](./imgs/有状态登录.png)
![无状态登录](./imgs/无状态登录.png)
![无状态登录原理](./imgs/无状态登录原理.png)
**Session vs JWT**

- Session 需要保存在服务器上，而 Session ID 则保存在前端 cookie 中
- JWT 信息只需要保存在客户端
- 无状态登录优势：分布式部署(这是个啥？)

#### 10-8【jwt 实例】JWT 与单点登录实例解释

就不做笔记了，人家的视频和电子教程已经讲述得非常好了

#### 10-11【redux-persist】登录持久化

解决 页面一刷新，登录的数据就消失的问题

**Cookie、Session 和 Web Storage**

- cookie 和 web Storage 保存在浏览器中；session 保存于服务器上
- 单个 cookie 保存的数据不能超过 4k，web Storage 上限 5MB 以上；session 无上限
- cookie 和 web Storage 安全性差；session 性能差
- cookie 会在同源的 http 请求中会被自动携带; web Stroage 不会自动发送

由于项目使用的是无状态登录：JWT

- 只能选择使用 cookie 或者 web storage 来保存
- 在这个项目中，选择 web storage 来做数据的持久化

**Web Storage 好处**

- 有效降低网络流量，一旦数据保存在本地之后，就可以避免再次向服务器发送请求，减少不必要的请求，降低数据在服务器和浏览器之间不必要的来回传递
- 快速显示数据(比在网络请求速度要快很多)
- 用作临时存储
...
对于 redux 的持久化，有个简单的插件可以来使用 `redux-persist`

> 但这个的确算是一种方法！！之后可以跟 jira 项目的数据持久化方法比较比较
...
1. 如何使用 react-router-dom 来配置私有路由？(私有路由是啥？感觉有点厉害)
2. 如何跨 slice 处理 action？
...
### 放在慕课网答疑区的疑惑解答 以及 突然想到的一些问题：

#### axios 的 基本使用 (全部教程：https://www.bilibili.com/video/BV1wr4y1K7tq?p=4)
...
似乎!important 优先级是大于内联样式的

1. ![慕课问题1](./imgs/慕课问题1.png)
1. ![慕课问题2](./imgs/慕课问题2.png)
